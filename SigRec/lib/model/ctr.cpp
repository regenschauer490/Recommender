	...
	double z)
{
	...
	sigdm::impl::normalize_dist_v(x_proj); // fix the normaliztion issue due to numerical errors
}
	...
	V3 const& opt_x)
{
	...
	g.array() *= -1;
#else
	sigdm::impl::compound_assign_v([](double& v1){ v1 *= -1; }, g);
#endif
...
double f_simplex(
	...
	V2 const& opt_x)
{
	V1 y = sigdm::impl::map_v([&](double x){ return safe_log(x); }, opt_x);
	V1 z = v - opt_x;

	double f = sigdm::impl::inner_prod(y, gamma);	
	double val = sigdm::impl::inner_prod(z, z);
	...
	V2&& opt_x)
{
	...
	double r = 0.5 * sigdm::impl::inner_prod(g, x_bar);
...
void CTR::init()
{
	...
	if (hparam_->beta_.empty()){
		for(TopicId k = 0; k < K_; ++k){
			auto&& beta_k = sigdm::impl::row_(beta_, k);
			for (ItemId v = 0; v < V_; ++v) {
				beta_k(v) = randf();
			}
			sigdm::impl::normalize_dist_v(beta_k);
		}
	}
	else{
	...
	sigdm::impl::set_zero(user_factor_, U_, K_);
	sigdm::impl::set_zero(item_factor_, I_, K_);

	if (!hparam_->theta_opt_){
		for (ItemId i = 0; i < I_; ++i){
			auto&& if_v = sigdm::impl::row_(item_factor_, i);
...
const sig::FilepathString item_factor_fname = SIG_TO_FPSTR("ctr_item_factor");
const sig::FilepathString user_factor_fname = SIG_TO_FPSTR("ctr_user_factor");
const sig::FilepathString theta_fname = SIG_TO_FPSTR("ctr_theta");

template <class M>
void save_mat(sig::FilepathString path, M const& mat) 
{
	std::ofstream ofs(path);

	if (ofs.is_open()) {
		for (uint i = 0, size1 = sigdm::impl::size_row(mat); i < size1; ++i) {
			for (uint j = 0, size2 = sigdm::impl::size(sigdm::impl::row_(mat, i)); j < size2; ++j) ofs << mat(i, j) << " ";
			ofs << std::endl;
		}
	}
	else std::cout << "saving file failed: " << sig::to_string(path) << std::endl;
};

template <class M>
void load_mat(sig::FilepathString path, M& mat)
{
	auto tmp = sig::load_num2d<double>(path, " ");

	if (tmp) {
		for (uint i = 0, size1 = sigdm::impl::size_row(mat); i < size1; ++i) {
			auto&& row = sigdm::impl::row_(mat, i);
			for (uint j = 0, size2 = sigdm::impl::size(row); j < size2; ++j)  row(j) = (*tmp)[i][j];
		}
		std::cout << "loading file: " << sig::to_string(path) << std::endl;
	}
};
...
void CTR::save() const
{
	...
	auto base_path = input_data_->getWorkingDirectory(); //+ SIG_TO_FPSTR("params/");
	auto mid = model_id_ >= 0 ? sig::to_fpstring(model_id_) : SIG_TO_FPSTR("");

	save_mat(base_path + item_factor_fname + mid, item_factor_);
	save_mat(base_path + user_factor_fname + mid, user_factor_);
	save_mat(base_path + theta_fname + mid, theta_);
...
void CTR::load()
{
	...
	load_mat(base_path + item_factor_fname + mid, item_factor_);
	load_mat(base_path + user_factor_fname + mid, user_factor_);
	load_mat(base_path + theta_fname + mid, theta_);
}
...
double CTR::docInference(ItemId id,	bool update_word_ss)
{
	...
	double likelihood = 0;
	auto const theta_v = sigdm::impl::row_(theta_, id);
	auto log_theta_v = sigdm::impl::map_v([&](double x){ return safe_log(x); }, theta_v);
	
	for (auto tid : item_tokens_[id]){
		WordId w = tokens_[tid].word_id;
		auto&& phi_v = sigdm::impl::row_(phi_, tid);

		for (TopicId k = 0; k < K_; ++k){
			phi_v[k] = theta_v[k] * sigdm::impl::at_(beta_, k, w);
		}
		sigdm::impl::normalize_dist_v(phi_v);
	...
	if (pseudo_count > 0) {
		likelihood += pseudo_count * sigdm::impl::sum_v(log_theta_v);
	}

	// smoothing with small pseudo counts
	sigdm::impl::assign_v(gamma_, pseudo_count);
	
	for (auto tid : item_tokens_[id]){
		for (TopicId k = 0; k < K_; ++k) {
			//double x = doc->m_counts[tid] * phi_(tid, k);	// doc_word_ct only
			double const& x = sigdm::impl::at_(phi_, tid, k);
			gamma_[k] += x;
			
			if (update_word_ss){
				sigdm::impl::at_(word_ss_, k, tokens_[tid].word_id) -= x;
			}
		}
	}
...
{ 
	...
	BlasMatrixKK XX = sigdm::impl::make_zero<BlasMatrixKK>(K_, K_); //BlasMatrixKK::Zero(K_, K_);

	// calculate VCV^T in equation(8)
	for (uint i = 0; i < I_; i ++){
		if (std::begin(item_ratings_[i]) != std::end(item_ratings_[i])){
			auto const& vec_v = sigdm::impl::row_(item_factor_, i);

			XX += sigdm::impl::outer_prod(vec_v, vec_v);
		}
    }
	...
	for (uint j = 0; j < U_; ++j){
		...
		if (std::begin(ratings) != std::end(ratings)){
			BlasMatrixKK A = XX;
			BlasVectorK x = sigdm::impl::make_zero<BlasVectorK>(K_);

			for (auto rating : ratings){
				auto const& vec_v = sigdm::impl::row_(item_factor_, rating->item_id_);

				for (uint m = 0; m < K_; ++m) {
					for (uint n = 0; n < K_; ++n) sigdm::impl::at_(A, m, n) += delta_ab * vec_v[m] * vec_v[n];
				}
				//A += delta_ab * vec_v.transpose() * vec_v;
				x += hparam_->a_ * vec_v;
			}

			auto vec_u = sigdm::impl::row_(user_factor_, j);
			...
			auto result = sigdm::impl::inner_prod(vec_u, vec_u);
...
{
	double delta_ab = hparam_->a_ - hparam_->b_;
	BlasMatrixKK XX = sigdm::impl::make_zero<BlasMatrixKK>(K_, K_);
	
	for (uint j = 0; j < U_; ++j){
		if (std::begin(user_ratings_[j]) != std::end(user_ratings_[j])){
			auto const& vec_u = sigdm::impl::row_(user_factor_, j);
			XX += sigdm::impl::outer_prod(vec_u, vec_u);
		}
	}
	...
	XX.array() *= hparam_->b_;
#else
	sigdm::impl::compound_assign_m([&](double& v) { v *= hparam_->b_; }, XX);
#endif
		
	for (uint i = 0; i < I_; ++i){
		auto&& vec_v = sigdm::impl::row_(item_factor_, i);
		auto const theta_v = sigdm::impl::row_(theta_, i);
		...
		if (std::begin(ratings) != std::end(ratings)){
			...
			for (auto rating : ratings){
				auto const& vec_u = sigdm::impl::row_(user_factor_, rating->user_id_);

				for (uint m = 0; m < K_; ++m) {
					for (uint n = 0; n < K_; ++n) sigdm::impl::at_(A, m, n) += delta_ab * vec_u[m] * vec_u[n];
				}
			...
			BlasMatrixKK B = A;		// save for computing likelihood 
...
#else
			likelihood_ += -0.5 * sigdm::impl::inner_prod(vec_v, boost::numeric::ublas::prod(B, vec_v));
#endif
			// likelihood part of theta, even when theta=0, which is a special case
			BlasVectorK x2 = vec_v;
			
			//sig::for_each_v([](double& v1, double v2){ v1 -= v2; }, x2, theta_v);
			x2 -= theta_v;

			auto result = sigdm::impl::inner_prod(x2, x2);
			likelihood_ += -0.5 * hparam_->lambda_v_ * result;

			if (hparam_->theta_opt_){
				likelihood_ += docInference(i, true);
				optimize_simplex(gamma_, vec_v, hparam_->lambda_v_, sigdm::impl::row_(theta_, i));
			}
		}
		else{
			...
			if (hparam_->theta_opt_) {
				...
				sigdm::impl::normalize_dist_v(gamma_);
				sigdm::impl::row_(theta_, i) = gamma_;
			}
		}
	}
}
...
void CTR::updateBeta()
{
	...
	for (TopicId k = 0; k < K_; ++k){
		auto beta_v = sigdm::impl::row_(beta_, k);

		sigdm::impl::normalize_dist_v(beta_v);
		sigdm::impl::row_(log_beta_, k) = sigdm::impl::map_v([&](double x){ return safe_log(x); }, beta_v);
	}
}
...
void CTR::train(uint max_iter, uint min_iter, sig::Maybe<FilepathString> info_saved_dir, bool is_save_parameter)
{
	uint iter = 0;
	double likelihood_old;
	sig::ManageConvergenceSimple conv(conv_epsilon_);

	auto base_path = info_saved_dir ? sig::modify_dirpath_tail(*info_saved_dir, true) : input_data_->getWorkingDirectory();
	...
	auto info_print = [&](uint iter, double likelihood, double converge){
		...
		sig::save_line(txt, base_path + SIG_TO_FPSTR("iteration_info.txt") + model_id, sig::WriteMode::append);
	};

	// キャッシュ用の領域確保（今後、trainの終了判定をユーザが設定できるよう変更する場合、キャッシュの再確保を行わないように変更）
	if (hparam_->enable_recommend_cache_) estimate_ratings_ = MatrixUI<Maybe<double>>(U_, VectorI<Maybe<double>>(I_, Nothing(double{})));

	if (max_iter < min_iter) std::swap(max_iter, min_iter);

	if (hparam_->theta_opt_){
		gamma_ = sigdm::impl::make_zero<BlasVectorK>(K_);
		log_beta_ = sigdm::impl::map_m([&](double x){ return safe_log(x); }, beta_);
		word_ss_ = BlasMatrixKV(K_, V_); // SIG_INIT_MATRIX(double, K, V, 0);
		phi_ = BlasMatrixTK(T_, K_);  //SIG_INIT_MATRIX(double, T, K, 0);
	}
...
inline double CTR::estimate(UserId u_id, ItemId i_id) const
{
	...
	if (!estimate_ratings_) {
		auto uvec = sigdm::impl::row_(user_factor_, u_id);
		auto ivec = sigdm::impl::row_(item_factor_, i_id);
		return sigdm::impl::inner_prod(uvec, ivec);
	}
	else if(!(*estimate_ratings_)[u_id][i_id]) {
		//return inner_prod(sigdm::impl::row_(user_factor_, u_id), sigdm::impl::row_(item_factor_, i_id));
		auto uvec = sigdm::impl::row_(user_factor_, u_id);
		auto ivec = sigdm::impl::row_(item_factor_, i_id);
		(*estimate_ratings_)[u_id][i_id] = sigdm::impl::inner_prod(uvec, ivec);
	}
...
auto CTR::getWordOfTopic(TopicId k_id, uint num_get_words, bool use_term_score) const->std::vector< std::tuple<std::wstring, double>>
{
	return calcTopWords(
		use_term_score ? getTermScore(k_id) : getPhi(k_id),
		num_get_words,
		*input_data_->getWordSet()
	);
}
